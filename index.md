...
<div id="gameContainer">
    ...
    <div id="ui">
        <div class="stat">❤️ <span id="lives">5</span></div>
        <div class="stat">⭐ <span id="score">0</span></div>
        <div class="stat">🎯 Level <span id="level">1</span></div>
        <div class="stat">🛡️ <span id="shields">0</span></div>
        <div class="stat">💎 <span id="specialCount">0</span>/12</div>
    </div>
    <div class="progress-container">
        <div class="progress-bar"><div class="progress-fill" id="progressFill"></div></div>
    </div>
    <div class="boss-health" id="bossHealth">
        <div class="boss-name">💃 Prancercise Lady</div>
    ...
    <div class="swipe-hint">Swipe ⬅️➡️ to move • Tap to throw poles 🥢</div>

```
<div id="startScreen" class="screen-overlay">
    <h1>🚶‍♀️ Michele's Pole Walking Adventure 🚶‍♀️</h1>
    ...
    <div class="instructions">
        ...
        <p>• Michele walks forward automatically</p>
        <p>• <strong>Swipe left/right</strong> to change lanes</p>
        <p>• <strong>Tap anywhere</strong> to shoot your pole!</p>
        <p>• Poles shoot up and destroy enemies in your lane</p>
        <p>• Dodge sweatbands from the Prancercise Lady</p>
        <p><strong>Collect Items:</strong></p>
        <p>⭐ Stars = Points &nbsp; 🥢 Poles = Upgrades</p>
        <p>💎 Secrets = Big bonus (collect all 12!)</p>
        <p>❤️ Extra lives &nbsp; 🛡️ Shields = Protection</p>
...
const py   = (player.y - 35) - game.scrollY;  // screen → world Y
...
if(level===5){
    ...
    // ── SECRETS  – 3 per level, placed in first 30 % ──
...
```
    ...
    proj.y += proj.vy;          // vy is -18 → y decreases → moves up
        ...
        if(Math.abs(proj.x - boss.x)<55 &&
           ...
           projScreenY   < bScreenY+80){
            ...
            if(boss.health<=0){
                ...
                else { showMessage('Prancercise Lady Defeated! 💃'); playVictory(); }
            }
        }
    }
...
playSecret();
showMessage(‘Secret Found! 💎 +500  (’+game.specialItemsCollected+’/12)’);
break;
case ‘life’:
if(game.lives<game.maxLives){
game.lives++; document.getElementById(‘lives’).textContent=game.lives;
playExtraLife(); showMessage(‘Extra Life! ❤️’);
...
if(game.shields>0){
game.shields–; document.getElementById(‘shields’).textContent=game.shields;
showMessage(‘Shield Absorbed Hit! 🛡️’); playShield();
...
document.getElementById(‘levelCompleteText’).textContent=‘Level ‘+game.level+’ Complete! 🎉’;
...
bonusCtx.fillText(“Jonathan’s Revenge!”,bonusCanvas.width/2,115);
...
// draw order: items → jonathan → enemies → boss → projectiles → player
drawItems();
drawJonathan();
